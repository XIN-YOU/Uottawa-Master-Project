...
for(k in 1:numsim){
  ...
  minindex<-min(gxe.add$pval[1:12,2])
  newmat[k,]<-c(minindex)
  
  
}

write.table(newmat, paste("MTHFR",jobindex,"_","min-p",".txt",sep=""),quote=F,row=F,col.names = T,append = F)
