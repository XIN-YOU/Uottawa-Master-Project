...
for(k in 1:3){
  ...
  ############################################################################################ 
  ############################################################################################
  snp=samplegenomes(nsnp=1,ngene=1,npathway=1)
  ...
  for(k in 1:3){
    maternal_genotype[[k]]=geno_list[[k]][mather_index, snp]
  for(i in 1:length(maternal_genotype[[k]])){
    y[i]=maternal_genotype[[k]][i]*envvar[i]
    odds[i]=exp(b1+b2*maternal_genotype[[k]][i]+b3*envvar[i]+b4*y[i])
...
write.table(disease_genomat_finalfinal,"/Users/Shelly/Documents/Project_R_code/MATERNAL/disease_genotype.ped",
            quote=F,col.names = T,row.names = T)
  


disease.data=read.table("/Users/Shelly/Documents/Project_R_code/MATERNAL/disease_genotype.ped",header = T)
...
write.csv(thedata,"/Users/Shelly/Documents/Project_R_code/MATERNAL/diseaseparents_e=0.csv")
dad.index=vector()
mom.index=vector()
...
#[1] 0.03888889
#[1] 0.1888889
#[1] 0.08333333 this is the probabilities when environment=0;
...
therow=sort(c(row1,row2))
thedata=disease.data[therow,]
write.csv(thedata,"/Users/Shelly/Documents/Project_R_code/MATERNAL/diseaseparents_e=1.csv")
dad.index=vector()
mom.index=vector()
dad.index=seq(1,length(en.index),2)
mom.index=dad.index+1
...
#[1] 0.01875
#[1] 0.046875
#[1] 0.059375 this is the probability when environment=1
...
samplegenomes=function(nsnp,ngene,npathway){
  ...
  if (nsnp==1&&ngene==2&&npathway==1){
    ...
    for(i in 1:2){
      lsnp[i]=sample(x=as.numeric(as.character(geno[i,3])):as.numeric(as.character(geno[i,4])),size=1) # sample snps
      return(lsnp)
    }
  }
