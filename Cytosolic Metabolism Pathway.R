...
for(k in 1:numsim){
  ...
  minindex<-min(gxe.add$pval[1:56,2],gxe.add$pval[63:283,2],gxe.add$pval[450:492,2],
                gxe.add$pval[499:529,2],gxe.add$pval[545:559,2])
  newmat[k,]<-c(minindex)
  
  
}

write.table(newmat, paste("pathway",jobindex,"_","min-p",".txt",sep=""),quote=F,row=F,col.names = T,append = F)
